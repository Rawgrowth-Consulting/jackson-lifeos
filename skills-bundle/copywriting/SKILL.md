...
```bash
cat brand/identity/12-brand-positioning.md
cat brand/voice.md
cat ops/strategy/offer/undeniable-offer.md
cat brand/identity/02-icp.md
cat brand/identity/01-personal-profile.md
cat sales/copy-library/principles/story-bank.md
cat brand/
```
...
```bash
cat sales/copy-library/COPY-ANALYSIS.md 2>/dev/null || echo "No copy analysis file found, proceed with brand docs only"
```
...
export $(grep -E '^(SUPABASE_URL|SUPABASE_SERVICE_KEY)=' /Users/scanbot/RawgrowthOS/.env | xargs)
...
```bash
ls sales/copy-library/ 2>/dev/null && echo "Local examples found"
...
2. **Audience:** Specific segment of ICP this targets. Pull from `brand/identity/02-icp.md`
3. **Core message:** One sentence. What's the single takeaway?
4. **Proof points:** Pull real numbers and results from `brand/`. Never fabricate.
