...
| Matrix access token | syt_YWdlbnRib3Q_sXJzNktpYuSUJkCIufqd_2IFHMI |
| Matrix bot user ID | @agentbot:rawgrowth.local |
| Chris Slack user ID | U0ALN0C29H8 |
| Alex Slack ID | U0APFEVB92S |
| Dilan Slack ID | U0ANZCMHQP3 |
...
```bash
source ~/.zshrc
PROJECT_ROOT=$(git -C ~/RawgrowthOS rev-parse --show-toplevel 2>/dev/null || echo ~/RawgrowthOS)
...
Use direct Matrix API via curl. The bot user (@agentbot:rawgrowth.local) is already joined to bridged rooms.
...
BOT_ID = "@agentbot:rawgrowth.local"
...
Load brand voice: read brand/voice.md
...
```bash
PROJECT_ROOT=$(git -C ~/RawgrowthOS rev-parse --show-toplevel 2>/dev/null || echo ~/RawgrowthOS)
CHAT_ID=$(sqlite3 "$PROJECT_ROOT/store/rawgrowthos.db" "SELECT chat_id FROM sessions LIMIT 1;")
sqlite3 "$PROJECT_ROOT/store/rawgrowthos.db" "INSERT INTO hive_mind (agent_id, chat_id, action, summary, artifacts, created_at) VALUES ('comms', '$CHAT_ID', 'comms_run', 'Ran /comms. Sent N replies across Slack/Matrix bridges. Skipped N.', NULL, strftime('%s','now'));"
```
...
| Draft seems off-tone | Re-read brand/voice.md and redraft |
